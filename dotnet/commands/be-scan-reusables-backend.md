---
name: be-scan-reusables-backend
description: Pre-task intelligence agent for ASP.NET Core backend. Reads extracted docs from docs/modules/ to find reusable services, interfaces, DTOs, and helpers relevant to a task. Falls back to source scan only if docs are missing. Run before implementing anything new.
---
...
2. Include domain synonyms (e.g. "auth" → also "authorize", "claims", "token")
...
- `docs/modules/_global.md` — shared services, middleware, exception types
...
- API routes and response types
- DTOs and their fields
- Auth requirements applied
- Exported helpers
...
Only run this if `docs/modules/` does not exist. Prompt the user to run `/be-extract-modules` first, then continue with source scan as a best-effort.

### 3a — Detect backend root
- `Api/Modules/` → use this
- `src/Modules/` → use this

### 3b — Keyword filter on paths (no file reads yet)
Glob paths only:
```
<backend-root>/*/Helpers/**/*.cs
<backend-root>/*/**/*Service.cs
<backend-root>/*/**/*Controller.cs
<backend-root>/*/Dtos/**/*.cs
<backend-root>/*/**/*ServiceExtensions.cs
```
...
### Suggested Imports / Injections
​```csharp
// Constructor injection
private readonly IServiceName _service;

// DI registration in ServiceExtensions
services.AddScoped<IServiceName, ServiceName>();
​```
```

Omit any section that has no results.
After printing, add: "Run `/be-check-conventions-backend` after implementation to validate structure."
If docs were missing, add: "Tip: run `/be-extract-modules` to enable fast doc-based scanning."
