...
Example: `/fe-analyze-project src=frontend out=reports` → `src=frontend`, `out=reports`.
...
Invoke `Skill: fe-extract-features` with argument `src=<src>`.
...
2. If it does not exist, invoke `Skill: fe-analyze-feature` with arguments: `feature=<slug> src=<src> out=<out> index=<index>`
...
Invoke `Skill: fe-cross-analysis` with argument `out=<out>`.
...
Invoke `Skill: fe-final-report` with argument `out=<out>`.
