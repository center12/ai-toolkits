---
name: fe-scan-reusables-frontend
...
Only run this if `docs/features/` does not exist. Prompt the user to run `/fe-extract-features` first, then continue with source scan as a best-effort.
...
After printing, add: "Run `/fe-check-conventions-frontend` after implementation to validate structure."
If docs were missing, add: "Tip: run `/fe-extract-features` to enable fast doc-based scanning."
