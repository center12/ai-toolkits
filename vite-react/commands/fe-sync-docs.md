...
Run the `fe-extract-features` command with no arguments (extracts all features).
...
Tip: Run /fe-sync-docs --cron daily to keep docs in sync automatically.
```
...
- Prompt: "Run /fe-extract-features then /be-extract-modules to sync project documentation."
...
Manage:
  /fe-sync-docs --list           → view all scheduled syncs
  /fe-sync-docs --cancel <id>    → cancel this sync
```
...
Filter for triggers whose prompt contains "fe-extract-features" or "sync" or "docs".
...
Run /fe-sync-docs --cancel <id> to remove a schedule.
```

If none found, print: "No scheduled doc syncs found. Run `/fe-sync-docs --cron daily` to create one."
...
Run /fe-sync-docs --cron <interval> to reschedule.
```
