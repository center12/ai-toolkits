...
Example: `/analyze-project src=app out=reports` → `src=app`, `out=reports`.
...
Run `/extract-modules` with no arguments to extract docs for all modules.

After completion, glob `docs/modules/*.md` (excluding `_global.md`) to get the module list.
Parse each filename to extract the module slug.
...
For each module slug in the list, in order (NOT in parallel):

1. Check if `<out>/<index>-<slug>.md` already exists using Glob. If it exists, skip this module (log: "Skipping <slug> — already analyzed").
2. If it does not exist, read `docs/modules/<slug>.md` and the module source files. Produce a detailed analysis file at `<out>/<index>-<slug>.md` using the template below.
